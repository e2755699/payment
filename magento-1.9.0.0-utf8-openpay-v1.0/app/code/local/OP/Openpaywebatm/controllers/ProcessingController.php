...
class OP_Openpaywebatm_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpaywebatm/success';
    protected $_failureBlockType = 'openpaywebatm/failure';
    protected $_cancelBlockType = 'openpaywebatm/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpaywebatm_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpaywebatm_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpaywebatm_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '4', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpaywebatm')->getPendingPaymentStatus();
    }
}
