...
class OP_Openpayfami_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpayfami/success';
    protected $_failureBlockType = 'openpayfami/failure';
    protected $_cancelBlockType = 'openpayfami/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpayfami_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpayfami_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpayfami_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '12', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpayfami')->getPendingPaymentStatus();
    }
}
