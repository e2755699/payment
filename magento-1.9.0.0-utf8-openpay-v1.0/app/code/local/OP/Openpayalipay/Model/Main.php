...
class OP_Openpayalipay_Model_Main extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'openpayalipay_main';
    ...
    {
          return Mage::getUrl('openpayalipay/processing/redirect');
    }
    ...
    $txid = $this->getOrder()->getRealOrderId(); //賣家交易單號
    	...
    	$params = 	array(
            ...
            'cname'	=>	$cname,
            'xname'	=>	$xname,
						'return_url'	=> Mage::getUrl('openpayalipay/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('openpayalipay API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('openpayalipay API connection error. The request has not been processed.'));
		}
