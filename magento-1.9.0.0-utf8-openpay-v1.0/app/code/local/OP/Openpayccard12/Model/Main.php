...
class OP_Openpayccard12_Model_Main extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'openpayccard12_main';
    ...
    {
          return Mage::getUrl('openpayccard12/processing/redirect');
    }
	...
	$caddress	= $this->getOrder()->getBillingAddress()->getData('street');
    $xaddress = $this->getOrder()->getShippingAddress()->getData('street');
    ...
    $iid = '12';
    	$params = 	array(
            ...
            'cname'	=>	$cname,
            'xname'	=>	$xname,
						'return_url'	=> Mage::getUrl('openpayccard12/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('openpayvacc API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('openpayvacc API connection error. The request has not been processed.'));
		}
