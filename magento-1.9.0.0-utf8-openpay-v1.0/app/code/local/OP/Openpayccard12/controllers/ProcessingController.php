...
class OP_Openpayccard12_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpayccard12/success';
    protected $_failureBlockType = 'openpayccard12/failure';
    protected $_cancelBlockType = 'openpayccard12/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpayccard12_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpayccard12_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpayccard12_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '1', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpayccard12')->getPendingPaymentStatus();
    }
}
