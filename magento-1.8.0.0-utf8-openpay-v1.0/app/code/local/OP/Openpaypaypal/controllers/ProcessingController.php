...
class OP_Openpaypaypal_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpaypaypal/success';
    protected $_failureBlockType = 'openpaypaypal/failure';
    protected $_cancelBlockType = 'openpaypaypal/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpaypaypal_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpaypaypal_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpaypaypal_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '11', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpaypaypal')->getPendingPaymentStatus();
    }
}
