...
class OP_Openpaycstore_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpaycstore/success';
    protected $_failureBlockType = 'openpaycstore/failure';
    protected $_cancelBlockType = 'openpaycstore/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpaycstore_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpaycstore_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpaycstore_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '8', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpaycstore')->getPendingPaymentStatus();
    }
}
