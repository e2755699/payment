...
class OP_Openpaytenpay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpaytenpay/success';
    protected $_failureBlockType = 'openpaytenpay/failure';
    protected $_cancelBlockType = 'openpaytenpay/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpaytenpay_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpaytenpay_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpaytenpay_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '14', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpaytenpay')->getPendingPaymentStatus();
    }
}
