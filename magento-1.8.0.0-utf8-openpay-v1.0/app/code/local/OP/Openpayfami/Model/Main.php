...
class OP_Openpayfami_Model_Main extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'openpayfami_main';
    ...
    {
          return Mage::getUrl('openpayfami/processing/redirect');
    }
    ...
    $txid = $this->getOrder()->getRealOrderId(); //賣家交易單號
    	...
    	$params = 	array(
            ...
            'cname'	=>	$cname,
            'xname'	=>	$xname,
						'return_url'	=> Mage::getUrl('openpayfami/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('openpayfami API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('openpayfami API connection error. The request has not been processed.'));
		}
