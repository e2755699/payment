...
class OP_Openpaychinapay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpaychinapay/success';
    protected $_failureBlockType = 'openpaychinapay/failure';
    protected $_cancelBlockType = 'openpaychinapay/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpaychinapay_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpaychinapay_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpaychinapay_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '10', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpaychinapay')->getPendingPaymentStatus();
    }
}
