...
class OP_Openpayibon_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpayibon/success';
    protected $_failureBlockType = 'openpayibon/failure';
    protected $_cancelBlockType = 'openpayibon/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpayibon_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpayibon_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpayibon_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '9', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpayibon')->getPendingPaymentStatus();
    }
}
