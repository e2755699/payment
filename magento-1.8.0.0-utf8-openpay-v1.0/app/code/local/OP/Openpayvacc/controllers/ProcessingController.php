...
class OP_Openpayvacc_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'openpayvacc/success';
    protected $_failureBlockType = 'openpayvacc/failure';
    protected $_cancelBlockType = 'openpayvacc/cancel';
    ...
    public function responseAction()
    {
		...
		$p1 = Mage::getModel('OP_Openpayvacc_Model_Main')->getConfigData('p1');	
    $p2 = Mage::getModel('OP_Openpayvacc_Model_Main')->getConfigData('p2');	
    $mid = Mage::getModel('OP_Openpayvacc_Model_Main')->getConfigData('mid');	
	  
  
    $plain_text = implode( '|', array($p1, $request['txid'], $request['amount'], '2', $request['status'], $request['tid'],$p2) );
    ...
    {
        return Mage::helper('openpayvacc')->getPendingPaymentStatus();
    }
}