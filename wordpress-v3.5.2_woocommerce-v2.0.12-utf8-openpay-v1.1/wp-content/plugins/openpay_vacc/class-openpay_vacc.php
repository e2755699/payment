<?php
/**
 * openpay_vacc Payment Gateway
 * Plugin URI: http://www.openpay_vacc.com.tw/
 * Description: Openpay虛擬帳號收款模組
 * Version: 1.0

 * Plugin Name: Openpay 虛擬帳號
 * @class 		openpay_vacc
 * @extends		WC_Payment_Gateway
 */
include_once(dirname(__FILE__).'/../openpay/openpay_main.php');
include_once(dirname(__FILE__).'/../../../wp-includes/plugin.php');

add_action('plugins_loaded', 'openpay_vacc_gateway_init', 0);

function openpay_vacc_gateway_init() {

    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }

    class WC_openpay_vacc extends WC_Payment_Gateway {


            public function __construct() {
            $this->id = 'openpay_vacc';
            $this->icon = 'https://www.twv.com.tw/openpay/image/openpay_module_vac.gif';
            $this->has_fields = false;
            $this->method_title = __('openpay_vacc', 'woocommerce');
    ...
    function init_form_fields() {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('啟動 Openpay虛擬帳號 收款模組', 'woocommerce'),
                    'default' => 'yes'
                ),
                'title' => array(
                    ...
                    'default' => __('Openpay虛擬帳號 收款', 'woocommerce')
                ),
                'description' => array(
                    'title' => __('客戶訊息', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('', 'woocommerce'),
                    'default' => __('台灣里 Openpay - <font color=red>虛擬帳號 繳費</font>', 'woocommerce')
                ),
                'mid' => array(
        ...
        public function admin_options() {
            ?>
            <h3><?php _e('台灣里 Openpay 虛擬帳號 收款模組', 'woocommerce'); ?></h3>
            <p><?php _e('此模組可以讓您使用台灣里 Openpay 虛擬帳號收款功能', 'woocommerce'); ?></p>
...
function receipt_page($order_id) {
    ...
    function add_openpay_vacc_gateway($methods) {
        $methods[] = 'WC_openpay_vacc';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_openpay_vacc_gateway');
}
?>